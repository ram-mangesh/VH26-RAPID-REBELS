...
PIPELINE: RealEventPipeline | None = None
...
async def handle_set_rate(request):
    try:
        ...
        logger.info(f"handle_set_rate raw body: {raw}")
        # Fix missing quotes around JSON keys (nginx bug workaround)
        ...
        logger.error(f"handle_set_rate error: {e}", exc_info=True)
...
async def on_startup(app):
    global PIPELINE
    logger.info(f"on_startup called: PIPELINE is None = {PIPELINE is None}")
    if PIPELINE is None:
        PIPELINE = RealEventPipeline(num_workers=8)
        await PIPELINE.start()
    app["broadcast_task"] = asyncio.create_task(broadcast_state())
    logger.info("RealEventPipeline started on startup with Kafka consumer")
