...
function updateDashboard(state) {
  ...
  document.getElementById('currentRate').textContent = s.rate || 0;
...
function updateOverloadIndicator(state, mode) {
  const ind = document.getElementById('overloadIndicator');
...
function updateDecisionTrace(traces) {
  ...
  for (const t of traces.slice(0, 24)) {
    ...
    const se = t.shed_reason ? ` · <span style="opacity:.8">${t.shed_reason}</span>` : '';
...
async function toggleDemo() {
  ...
  if (autoDemoRunning) {
    ...
    addLog('info', 'Auto demo stopped');
    autoDemoRunning = false;
  } else {
    await resetPipeline();
    await fetch('/api/demo', { method: 'POST' });
    btn.textContent = '■ Demo Running';
    addLog('info', 'Auto demo started: baseline → spike → recovery');
